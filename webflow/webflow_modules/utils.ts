 ...
 */
export const cx = (style: CSSModules, ...classNames: string[]) =>
 ...
 */
export const cj = (...classNames: (string | boolean | undefined)[]) =>
  classNames.filter(Boolean).join(" ");

const UNESCAPED_CHARS = /(\\b|\\f|\\r\\n|\\n|\\r|\\t|\\v)/gm;

export const removeUnescaped = (value: string) =>
 ...
 */
export const replaceSelector = (selector: string, styles: CSSModules) => {
...
export const useLayoutEffect = (
  isServer ? () => undefined : React.useLayoutEffect
...
function isScriptPresent(regex: RegExp): boolean {
  return Array.from(document.scripts).reduce<boolean>(
...
export function extractElement<T extends React.JSXElementConstructor<any>>(
  ...
  function removeElementByType(
    _elements: React.ReactNode[]
  ): React.ReactNode[] {
